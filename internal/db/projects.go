...
func (db *DB) CreateProject(ctx context.Context, project *models.Project) error {
	query := `
		INSERT INTO projects (
			...
			tone, visual_style, aspect_ratio, voice_id, cta,
	...
	return db.QueryRowContext(
		...
		project.Tone, project.VisualStyle, project.AspectRatio, project.VoiceID,
...
func (db *DB) ListProjects(ctx context.Context, status string, limit, offset int) ([]models.Project, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&p.Tone, &p.VisualStyle, &p.AspectRatio, &p.VoiceID,
